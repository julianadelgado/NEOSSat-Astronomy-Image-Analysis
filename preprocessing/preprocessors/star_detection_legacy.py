    ...
    def run(self, image: np.ndarray, header, output_dir: Path) -> dict:
        ...
        with open(csv_path, mode="w", newline="") as f:
            ...
            for star, (x_star, y_star, ra_star, dec_star, flux_star) in enum:
                ...
                identified_obj = query_simbad(
                    full_coord_string, simbad_acceptance_radius
                )
                ...
                plot_data.append(
                    {
                        ...
                        "id": id_star,
                        "deviation": (
                            deviation_star if deviation_star != "not_found" else None
                        ),
                    }
                )
        ...
        for star in plot_data:
            ...
            if id_star != "not_found":
                ...
                ax.text(x_star + 5, y_star + 5, id_star, color="cyan", fontsize=6)
            ...
            if show_acceptance_radius:
                ...
                radius_coord = star_coord.directional_offset_by(
                    0 * units.deg, simbad_acceptance_radius
                )
                x_radius, y_radius = wcs.world_to_pixel(radius_coord)
                radius_pix = np.sqrt(
                    (x_radius - x_star) ** 2 + (y_radius - y_star) ** 2
                )

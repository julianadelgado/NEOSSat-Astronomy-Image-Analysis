...
def find_and_remove_duplicates(data_dir: Path, threshold: int, action: str, move_dir: Path, recursive: bool = False):
    """
    ...
    for file_path in files:
        ...
        if is_duplicate:
            duplicates_found += 1
            print(f"Duplicate found: {file_path.name} is similar to {original_file.name} (Distance: {min_dist})")
    ...
    print(f"Summary: Processed {len(files)} files. Found {duplicates_found} duplicates.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Clean up similar images from a directory.")
    parser.add_argument("--data-dir", type=str, default="data", help="Directory containing images to check.")
    parser.add_argument("--threshold", type=int, default=5, help="Hamming distance threshold for similarity (0-64, lower means more strict). Default 5.")
    parser.add_argument("--action", type=str, choices=["delete", "move"], default="move", help="Action to take on duplicates.")
    parser.add_argument("--move-dir", type=str, default="duplicates", help="Directory to move duplicates to (if action is move).")

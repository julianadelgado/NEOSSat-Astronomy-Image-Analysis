...
def mock_cadc():
    ...
    results.__getitem__ = MagicMock(side_effect=lambda key: (
        [obs_id] if key == 'observationID'
        else results if isinstance(key, list)
        else results if isinstance(key, slice)
        else row
    ))
    ...
    with patch("core.fits_handler.Cadc") as MockCadc, \
         patch("core.fits_handler.download_file", return_value="/tmp/fake_download.fits"), \
         patch("core.fits_handler.shutil.move", side_effect=fake_move):

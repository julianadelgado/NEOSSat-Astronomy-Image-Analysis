...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QPushButton, QListWidget, QMessageBox, QProgressBar,
                             QComboBox, QCheckBox, QDoubleSpinBox, QGroupBox, QStackedWidget,
                             QAbstractItemView, QListWidgetItem, QTableWidget, QTableWidgetItem, QHeaderView, QSizePolicy)
...
class SkyViewThread(QThread):
    ...
    def run(self):
        try:
            hdul_list = SkyView.get_images(position=self.target, 
                                           survey=[self.survey], 
...
class CadcSearchThread(QThread):
    ...
    results_found = pyqtSignal(list, object) # list of dicts, and raw table (optional, or just list)
        ...
        try:
            ...
            if self.is_raw_degrees:
                ...
                if len(parts) >= 2:
                    ...
                    self.error_occurred.emit("Invalid degree format. Use: RA DEC (e.g., 240.5 -30.2)")
                    return
            else:
            ...
            if coords is not None:
                # It's a coordinate
                result = cadc.query_region(coords, radius=self.radius * u.deg, collection='NEOSSAT')
            else:
                # Treat as object name
                result = cadc.query_region(self.target, radius=self.radius * u.deg, collection='NEOSSAT')
            ...
            for i, row in enumerate(result):
                 url = urls[i]
                 obs_id = str(row['observationID']) if 'observationID' in row.colnames else f"img{i}"
                 time_obs = str(row['time_observation']) if 'time_observation' in row.colnames else "N/A"
    ...
    def run(self):
        ...
        for i, item in enumerate(self.download_list):
            ...
            try:
                ...
                safe_target_folder = "".join([c if c.isalnum() else "_" for c in target])
    ...
    def run(self):
        ...
        try:
            ...
            if self.mode == 'years':
                ...
                if not self.year or not self.day: return
...
class DownloadTab(QWidget):
    def __init__(self, data_folder):
        ...
        self.source_combo.addItems([
            "AstroQuery Target Search"
            "NEOSSAT (FTP Archive Browser)",
    ...
    def init_neossat_query_ui(self):
        ...
        self.cadc_result_list.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
    ...
    def start_skyview_download(self):
        ...
        self.sv_thread = SkyViewThread(target, self.sv_survey.currentText(), 
                                       self.sv_radius.value(), self.data_folder)
    ...
    def load_neossat_years(self):
        ...
        self.thread_ftp_s.years_fetched.connect(lambda y: self.neo_year_combo.addItems(y))
    ...
    def start_cadc_download_selected(self):
        ...
        self.cadc_d_thread.progress_update.connect(lambda c, t: self.progress_bar.setValue(c))
...
class VisualizeTab(QWidget):
    def __init__(self, data_folder):
        ...
        self.header_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        ...
        try:
            with fits.open(filepath) as hdul:
                ...
                if data is not None:
                    ...
                    cax = ax.imshow(data, cmap='gray', vmin=vmin, vmax=vmax, origin='lower')
                    ...
                    QMessageBox.warning(self, "Error", "No valid image data found in FITS file.")
        except Exception as e:

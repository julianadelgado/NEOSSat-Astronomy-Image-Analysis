    ...
    def download_images_to_directory(self, directory):
        try:
            if self.date_obs is None:
                print("Observation date is not available. Cannot filter images by date.")
                return
            ...
            mask_date = [search_date_doy in str(obs_id) for obs_id in results['observationID']]
            ...
            mask = [('cord' in str(obs_id).lower()) for obs_id in results_filtered_date['observationID']]
            filtered_results = results_filtered_date[mask] if any(mask) else results_filtered_date
